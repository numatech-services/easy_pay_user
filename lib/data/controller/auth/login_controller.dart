...
import 'package:viserpay/view/screens/auth/login/widget/weak_password.dart';
  ...
  void loginUser() async {
    ...
    if (model.statusCode == 200) {
      ...
      if (loginModel.status.toString().toLowerCase() == "success") {
        ...
        final bool isWeak = loginModel.data?.weakPassword == true;

        if (isWeak) {
          print(
              "isWeak========================${loginModel.data?.weakPassword}");

          // On navigue d'abord pour avoir un contexte valide
          ...
          // Puis on affiche la modale par-dessus
          if (isWeak) {
             RouteMiddleWare.checkUserStatusAndGoToNextStep(
              user: loginModel.data?.user,
              accessToken: loginModel.data?.accessToken ?? '',

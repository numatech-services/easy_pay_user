...
class StringConverter {
 ...
 static String minus(String? first, String? last, {int precision = 2}) {
  try {
    // Vérifiez les nulls et retournez une valeur par défaut
    if (first == null || last == null) {
      print("Les valeurs first ou last sont nulles.");
      return "0.00";
    }

    num v1 = NumberFormat.decimalPattern().parse(first);
    num v2 = NumberFormat.decimalPattern().parse(last);

    return formatNumber("${v1 - v2}", precision: precision);
  } catch (e) {
    print("Erreur de format dans minus : $e");
    return "0.00";
  }
}
...
extension RemoveCommaFromNumber on String {
  String removeComma() => length > 0 ? replaceAll(',', '') : '';
}

extension StringCasingExtension on String {
  String toCapitalized() => length > 0 ? '${this[0].toUpperCase()}${substring(1).toLowerCase()}' : '';

  String toTitleCase() => replaceAll(RegExp(' +'), ' ').split(' ').map((str) {return str.toCapitalized();}).join(' ');
}
